...
//   try {
//     const offer = await Offer.findById(req.params.id).populate('category').populate('brand');
//     if (!offer) {
...
//     const updatedOffer = await Offer.findByIdAndUpdate(req.params.id, { name, category, brand, location, description, image, expire_at, discount_value }, { new: true });
//     if (!updatedOffer) {
//       return res.status(404).json({ error: 'Offer not found' });
//     }
